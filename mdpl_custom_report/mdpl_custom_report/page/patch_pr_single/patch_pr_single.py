...
@frappe.whitelist()
def patch_pr_fields(pr_name, items):
    if not frappe.has_permission("Purchase Receipt", "write"):
        frappe.throw(_("Not permitted"), frappe.PermissionError)
    ...
    if doc.docstatus != 1:
        frappe.throw(_("Only submitted Purchase Receipts can be patched via this tool."))

    changes = _apply_pr_patches(doc, items)

    if not changes:
        return {"status": "no_changes", "message": "No changes detected."}

    _finalize("Purchase Receipt", pr_name, changes)

    return {"status": "success", "message": "Purchase Receipt updated successfully.", "changes": changes}
...
@frappe.whitelist()
def bulk_patch_prs(pr_names, item_group_map, cost_center_map):
    if not frappe.has_permission("Purchase Receipt", "write"):
        frappe.throw(_("Not permitted"), frappe.PermissionError)
    ...
    for pr_name in pr_names:
        try:
            ...
            if doc.docstatus != 1:
                skip_count += 1
                errors.append({"pr": pr_name, "error": "Not submitted (docstatus={0})".format(doc.docstatus)})
                continue
            ...
            changes = _apply_pr_patches(doc, items_patch)

            if not changes:
                skip_count += 1
                results.append({"pr": pr_name, "changes": [], "status": "no_changes"})
                continue

            _finalize("Purchase Receipt", pr_name, changes)
            success_count += 1
            results.append({"pr": pr_name, "changes": changes, "status": "success"})

        except Exception as e:
            errors.append({"pr": pr_name, "error": str(e)})

    return {"success_count": success_count, "skip_count": skip_count, "errors": errors, "results": results}


# ------------------------------------------------------------------
# Internal helpers for Purchase Receipt
# ------------------------------------------------------------------

def _apply_pr_patches(doc, items):
    changes = []
    ...
    for incoming in items:
        ...
        if new_group and (row.item_group or "") != new_group:
            frappe.db.set_value("Purchase Receipt Item", row_name, "item_group", new_group, update_modified=False)
        ...
        if new_cc and (row.cost_center or "") != new_cc:
            frappe.db.set_value("Purchase Receipt Item", row_name, "cost_center", new_cc, update_modified=False)

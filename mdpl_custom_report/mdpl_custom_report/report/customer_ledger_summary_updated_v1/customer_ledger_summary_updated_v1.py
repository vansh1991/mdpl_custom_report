    ...
    def get_cheque_count(self):
        ...
        for row in results:
            if row.workflow_state == "Cheque Received":
                ...
                self.cheque_counts[row.party]["cheque_received_amount"] = row[
                    "total_amount"
                ]
            elif row.workflow_state == "Cheque Deposited":
                self.cheque_counts[row.party]["cheque_deposited_count"] = row["count"]
                self.cheque_counts[row.party]["cheque_deposited_amount"] = row[
                    "total_amount"
                ]
    ...
    def get_data(self):
        ...
        for party, row in self.party_data.items():
            if (
                ...
                or any(
                    row.get(f"avg_outstanding_{idx + 1}", 0)
                    for idx in range(len(self.ranges))
                )

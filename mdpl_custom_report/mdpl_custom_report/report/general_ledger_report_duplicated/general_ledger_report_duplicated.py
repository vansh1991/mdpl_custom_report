...
# ---------------------------------------------------------------------------
def get_party_gstin_map():
    """Returns {customer_name: gstin} for all customers that have a GSTIN."""
    rows = frappe.get_all("Customer", fields=["name", "gstin"])
    return {r.name: (r.gstin or "") for r in rows}
...
def get_result(filters, account_details):
    ...
    # -- NEW: if group_by_gst is set, route to GST-grouped builder ----------
...
def get_data_grouped_by_gst(filters, account_details, accounting_dimensions, gl_entries):
    """
    Groups GL entries by GSTIN (fetched from Customer master).
    - Entries for Customer party type with a GSTIN are grouped under a
      GSTIN header row with subtotals.
    - Entries without a GSTIN, or for non-Customer party types, are shown
      as normal individual rows under their own header.
    - Each group shows: header row (GSTIN + customer names), all GL entry
      rows, and a subtotal row.
    - A grand total closing row is appended at the end.
    """
    set_bill_no(gl_entries)

    party_gstin_map = get_party_gstin_map()  # {customer: gstin}

    # Attach gstin to each GL entry
    for gle in gl_entries:
        if gle.get("party_type") == "Customer" and gle.get("party"):
            gle["gstin"] = party_gstin_map.get(gle["party"], "")
        else:
            gle["gstin"] = ""
    ...
    # Separate opening entries (before from_date) from period entries
    opening_entries = []
    period_entries = []
    for gle in gl_entries:
        if gle.posting_date < from_date or (cstr(gle.is_opening) == "Yes" and not show_opening_entries):
    ...
    # Group period entries by GSTIN (or party if no GSTIN)
    # Key: gstin if available, else "__no_gstin__:<party>:<party_type>"
    group_order = []
    groups = {}

    for gle in period_entries:
        gstin = gle.get("gstin") or ""
        if gstin:
            key = f"gstin:{gstin}"
        else:
            party = gle.get("party") or ""
            party_type = gle.get("party_type") or ""
            key = f"no_gstin:{party_type}:{party}:{gle.get('account', '')}"
        ...
        groups[key]["entries"].append(gle)
        groups[key]["debit"] += gle.debit
        groups[key]["credit"] += gle.credit
        if gle.get("party_type") == "Customer" and gle.get("party"):
            cname = gle.get("party_name") or gle.get("party")
            groups[key]["customer_names"].add(cname)
    ...
    opening_row["account"] = f"'{_('Opening')}'"
    opening_row["account_currency"] = filters.get("account_currency")
    opening_row["presentation_currency"] = filters.get("presentation_currency")
    ...
    for key in group_order:
        ...
        # -- Group header row ------------------------------------------------
        if grp["gstin"]:
            header_label = f"GST: {grp['gstin']}"
            if grp["customer_names"]:
                header_label += "  |  " + ", ".join(sorted(grp["customer_names"]))
        else:
            if grp["party"]:
                header_label = grp["party_type"] + ": " + grp["party"]
            else:
                header_label = grp["account"] or "Other Entries"

        header_row = _dict({
            "account": header_label,
            "gstin": grp["gstin"],
            "posting_date": None,
            "voucher_no": None,
            "debit": None,
        ...
        # -- Individual GL entry rows ----------------------------------------
        for gle in grp["entries"]:
            gle["account_currency"] = filters.get("account_currency")
            gle["presentation_currency"] = filters.get("presentation_currency")
        ...
        # -- Group subtotal row ----------------------------------------------
        subtotal_row = _dict({
            "account": f"Subtotal - {grp['gstin'] or grp['party'] or 'Other'}",
            "gstin": grp["gstin"],
            "posting_date": None,
            "voucher_no": None,
            "debit": grp["debit"],
        ...
        grand_total_debit += grp["debit"]
        grand_total_credit += grp["credit"]

    # Grand total row
    total_row = _dict(DEBIT_CREDIT_DICT.copy())
    total_row["account"] = f"'{_('Total')}'"
    ...
    total_row["account_currency"] = filters.get("account_currency")
    total_row["presentation_currency"] = filters.get("presentation_currency")
    data.append(total_row)

    # Grand closing row
    closing_row = _dict(DEBIT_CREDIT_DICT.copy())
    closing_row["account"] = f"'{_('Closing (Opening + Total)')}'"
    ...
    closing_row["account_currency"] = filters.get("account_currency")
    closing_row["presentation_currency"] = filters.get("presentation_currency")

...
// Modified: Added GST Grouping (group_by_gst) filter

frappe.query_reports["General Ledger Report V1"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		// -- PAN filter --------------------------------------------------------
		{
			...
			fieldtype: "Data",
			default: "",
			description: "Filter entries by Customer PAN number.",
		},
		// -- Group by PAN filter ------------------------------------------------
		{
			fieldname: "group_by_gst",
			label: __("Group by PAN"),
			fieldtype: "Check",
			default: 0,
			description: "Groups Customer entries by PAN. Customers sharing the same PAN are merged under one header.",
		},
		// -- Subledger Grouping filter ------------------------------------------
		{
			fieldname: "group_by_subledger",
			label: __("Group by Subledger"),
			fieldtype: "Check",
			default: 0,
			description: "Groups entries by Account (subledger), with each Party shown as a sub-section under their account.",
		},
		// -----------------------------------------------------------------------
	],
...
erpnext.utils.add_dimensions("General Ledger Report V1", 15);
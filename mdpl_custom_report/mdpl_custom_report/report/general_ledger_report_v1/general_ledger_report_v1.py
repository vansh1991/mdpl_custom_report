...
# Modified: Added GST Grouping (group_by_gst) feature
...
# ---------------------------------------------------------------------------
# GST helper - build a party -> GSTIN map for Customer party type
# ---------------------------------------------------------------------------
def get_party_pan_map():
    """
    Returns {party_name: pan} for Customers using the custom 'pan' field
    on the Customer DocType (tabCustomer.pan).
    """
    result = {}
    for r in frappe.get_all("Customer", fields=["name", "pan"]):
        if r.pan:
            result[r.name] = r.pan
    return result
...
def get_result(filters, account_details):
    ...
    # -- PAN filter: resolve PAN -> list of matching customer names -----------
    if filters.get("pan"):
        pan_value = filters.get("pan").strip().upper()
        matched_customers = frappe.db.get_all(
            "Customer",
            filters={"pan": pan_value},
            pluck="name"
        )
        if not matched_customers:
            # No customers found for this PAN - return empty result
            return []
        # Inject party filter so GL query only fetches matching customers
        filters["party_type"] = "Customer"
        filters["party"]      = matched_customers
    # -------------------------------------------------------------------------

    gl_entries = get_gl_entries(filters, accounting_dimensions)

    # Route to appropriate builder based on active filter
    if filters.get("group_by_gst"):
        data = get_data_grouped_by_gst(filters, account_details, accounting_dimensions, gl_entries)
    elif filters.get("group_by_subledger"):
        data = get_data_grouped_by_subledger(filters, account_details, accounting_dimensions, gl_entries)
    else:
...
def initialize_gle_map(gl_entries, filters):
    ...
    for gle in gl_entries:
        ...
        if group_by_value not in gle_map:
            gle_map[group_by_value] = _dict(totals=get_totals_dict(), entries=[])
    return gle_map
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map):
    ...
    def update_value_in_dict(data, key, gle, show_net_values=False):
        ...
        if (
            ...
            and account_type_map.get(data[key].account) in ("Receivable", "Payable",)
        ) or show_net_values:
...
# ---------------------------------------------------------------------------
# NEW: GST-grouped data builder
# ---------------------------------------------------------------------------
def get_data_grouped_by_gst(filters, account_details, accounting_dimensions, gl_entries):
    """
    Groups GL entries by Customer PAN (tabCustomer.pan).

    Layout:
    -----------------------------------------------------------------------
    PAN: ABCDE1234F              Dr 1,80,000  Cr 0.00   (PAN header, bold)
        ABC 1                    Dr 1,00,000  Cr 0.00   (customer sub-header, bold)
          01-Jan  INV-001  Dr 1,00,000                  (GL entries, indented)
        ABC 2                    Dr   80,000  Cr 0.00   (customer sub-header, bold)
          03-Jan  INV-002  Dr   80,000
    -----------------------------------------------------------------------
    - PAN header shown only when 2+ customers share the same PAN.
    - Solo customer (unique PAN or no PAN): entries shown as normal rows.
    - Non-customer entries shown as normal rows.
    """
    set_bill_no(gl_entries)

    # {customer_name: pan}
    pan_map = get_party_pan_map()

    # Attach pan to each GL entry
    for gle in gl_entries:
        pt  = gle.get("party_type") or ""
        par = gle.get("party") or ""
        if pt == "Customer" and par:
            gle["gstin"] = pan_map.get(par) or ""
        else:
            gle["gstin"] = ""
    ...
    show_opening_entries = filters.get("show_opening_entries")
    currency  = filters.get("account_currency")
    pres_curr = filters.get("presentation_currency")

    opening_entries = []
    period_entries  = []
    for gle in gl_entries:
        if gle.posting_date < from_date or (cstr(gle.is_opening) == "Yes" and not show_opening_entries):
            opening_entries.append(gle)
        elif gle.posting_date <= to_date or (cstr(gle.is_opening) == "Yes" and show_opening_entries):
            period_entries.append(gle)

    grand_opening = _dict(DEBIT_CREDIT_DICT.copy())
    for gle in opening_entries:
        grand_opening.debit  += gle.debit
        grand_opening.credit += gle.credit

    # -- Build groups: same PAN -> same group ---------------------------------
    group_order = []
    groups = {}

    for gle in period_entries:
        pan        = gle.get("gstin") or ""   # stored in gstin field
        party      = gle.get("party") or ""
        party_type = gle.get("party_type") or ""

        if pan:
            grp_key = f"pan:{pan}"
        else:
            grp_key = f"no_pan:{party_type}:{party}:{gle.get('account','')}"
        ...
        grp = groups[grp_key]
        grp["total_debit"]  += gle.debit
        grp["total_credit"] += gle.credit

        if party:
            sub_key   = party
            sub_label = gle.get("party_name") or party
        else:
            sub_key   = gle.get("account") or "Other"
            sub_label = sub_key

        if sub_key not in grp["sub_groups"]:
            grp["sub_order"].append(sub_key)
            grp["sub_groups"][sub_key] = {
                "label":   sub_label,
                "entries": [],
                "debit":   0.0,
                "credit":  0.0,
            }
        grp["sub_groups"][sub_key]["entries"].append(gle)
        grp["sub_groups"][sub_key]["debit"]  += gle.debit
        grp["sub_groups"][sub_key]["credit"] += gle.credit

    # -- Build output rows ----------------------------------------------------
    data = []

    opening_row = _dict(DEBIT_CREDIT_DICT.copy())
    opening_row.update(grand_opening)
    opening_row["account"]               = _("Opening")
    opening_row["account_currency"]      = currency
    opening_row["presentation_currency"] = pres_curr
    opening_row["balance"]               = grand_opening.debit - grand_opening.credit
    data.append(opening_row)

    grand_total_debit  = grand_opening.debit
    grand_total_credit = grand_opening.credit
    running_balance    = grand_opening.debit - grand_opening.credit

    for grp_key in group_order:
        grp = groups[grp_key]
        if not grp["sub_order"]:
            continue

        pan           = grp["pan"]
        has_multi_sub = len(grp["sub_order"]) > 1

        if has_multi_sub:
            # ----------------------------------------------------------------
            # 2+ customers share this PAN: show PAN group header + sub-headers
            # ----------------------------------------------------------------
            sub_labels = [grp["sub_groups"][sk]["label"] for sk in grp["sub_order"]]

            # PAN group header
            data.append(_dict({
                "account":               f"PAN: {pan}  |  " + ",  ".join(sub_labels),
                "gstin":                 pan,
                "posting_date":          None,
                "voucher_no":            None,
                "debit":                 grp["total_debit"],
                "credit":                grp["total_credit"],
                "balance":               None,
                "is_group_header":       1,
                "bold":                  1,
                "account_currency":      currency,
            ...
            for sub_key in grp["sub_order"]:
                sub = grp["sub_groups"][sub_key]

                # Customer bold sub-header
                data.append(_dict({
                    "account":               f"    {sub['label']}",
                    "gstin":                 pan,
                    "posting_date":          None,
                ...
                # GL entry rows indented under customer
                for gle in sub["entries"]:
                    running_balance             += gle.debit - gle.credit
                    gle["balance"]               = running_balance
                    gle["account_currency"]      = currency
                    gle["presentation_currency"] = pres_curr
                    gle["indent"]                = 1
                    data.append(gle)

        else:
            # ----------------------------------------------------------------
            # Solo customer: plain rows, no header
            # ----------------------------------------------------------------
            for sub_key in grp["sub_order"]:
                sub = grp["sub_groups"][sub_key]
                for gle in sub["entries"]:
        ...
        grand_total_debit  += grp["total_debit"]
        grand_total_credit += grp["total_credit"]

    total_row = _dict(DEBIT_CREDIT_DICT.copy())
    total_row["account"]               = _("Total")
    total_row["debit"]                 = grand_total_debit  - grand_opening.debit
    total_row["credit"]                = grand_total_credit - grand_opening.credit
    total_row["balance"]               = None
    total_row["bold"]                  = 1
    total_row["account_currency"]      = currency
    total_row["presentation_currency"] = pres_curr
    data.append(total_row)

    closing_row = _dict(DEBIT_CREDIT_DICT.copy())
    closing_row["account"]               = _("Closing (Opening + Total)")
    closing_row["debit"]                 = grand_total_debit
    closing_row["credit"]                = grand_total_credit
    closing_row["balance"]               = grand_total_debit - grand_total_credit
    closing_row["bold"]                  = 1
    closing_row["account_currency"]      = currency
    closing_row["presentation_currency"] = pres_curr
...
# ---------------------------------------------------------------------------
# Subledger grouping: group by Account, sub-group by Party
# ---------------------------------------------------------------------------
def get_data_grouped_by_subledger(filters, account_details, accounting_dimensions, gl_entries):
    """
    Groups GL entries by Account, then by Party within each account.

    Layout:
    -----------------------------------------------------------------------
    Debtors - MDPL               Dr 1,80,000  Cr 0.00   (account header, bold)
        ABC 1                    Dr 1,00,000  Cr 0.00   (party sub-header, bold)
          01-Jan  INV-001  Dr 1,00,000                  (GL entries, indented)
        ABC 2                    Dr   80,000  Cr 0.00   (party sub-header, bold)
          03-Jan  INV-002  Dr   80,000

    Bank - MDPL                  Dr 50,000    Cr 0.00   (account header, bold)
          05-Jan  PAY-001  Dr 50,000                    (entries indented, no party sub-header)
    -----------------------------------------------------------------------
    ALL accounts get a header row.
    Accounts with parties show each party as a bold sub-header.
    Accounts without parties show entries directly under the account header.
    """
    set_bill_no(gl_entries)
    ...
    # -- Build groups: account -> party -> entries ----------------------------
    group_order = []
    groups = {}
    ...
    for grp_key in group_order:
        ...
        # Account header row (always shown for every account)
        data.append(_dict({
            "account":               grp["account"],
            "subledger_account":     grp["account"],
            "posting_date":          None,
            "voucher_no":            None,
            "debit":                 grp["total_debit"],
            "credit":                grp["total_credit"],
        ...
        for sub_key in grp["sub_order"]:
            ...
            if sub_key == "__no_party__":
                # No party: entries go directly under account header
                for gle in sub["entries"]:
                    running_balance             += gle.debit - gle.credit
                    gle["balance"]               = running_balance
                    gle["account_currency"]      = currency
                    gle["presentation_currency"] = pres_curr
                    gle["subledger_account"]     = grp["account"]
                    gle["indent"]                = 1
                    data.append(gle)
            else:
                # Party sub-header (bold, indented)
                data.append(_dict({
                    "account":               f"    {sub['label']}",
                    "subledger_account":     grp["account"],
                    "posting_date":          None,
                ...
                # GL entry rows indented under party
                for gle in sub["entries"]:
                    running_balance             += gle.debit - gle.credit
                    gle["balance"]               = running_balance
                    gle["account_currency"]      = currency
        ...
        # Blank separator between account groups
        data.append({
            "account": "",
            "debit":   None,
            "credit":  None,
...
def get_columns(filters):
    ...
    # -- NEW: GSTIN column shown when group_by_gst is enabled ----------------
    if filters.get("group_by_gst"):
        columns.insert(2, {
            "label": _("PAN"),
            "fieldname": "gstin",
    ...
    # -- Subledger column shown when group_by_subledger is enabled -----------
    if filters.get("group_by_subledger"):
        columns.insert(2, {
            "label": _("Subledger Account"),
            "fieldname": "subledger_account",
            "fieldtype": "Data",
            "width": 200,
        })
    # -------------------------------------------------------------------------
